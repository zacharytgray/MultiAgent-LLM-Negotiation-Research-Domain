...
def build_tables(paths: List[str], k_history: int = 8) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for tid, steps in trajectories.items():
        # Sort by turn
        def get_turn(r):
            # Prefer explicit 'turn' field, fallback to state.timestep
            if 'turn' in r: return r['turn']
            if 'state' in r and 'timestep' in r['state']: return r['state']['timestep']
            return -1
        ...
        # Check success
        # Success if any step has action.type == "ACCEPT"
        has_accept = any(s.get('action', {}).get('type') == 'ACCEPT' for s in steps)
        ...
        if not is_success and not is_failure:
            # Trajectory is incomplete or indeterminate - Skip
            continue
        ...
        # Maintain local running history for this trajectory
...
def validate(df_outcomes: pd.DataFrame, df_steps: pd.DataFrame):
    print("\n--- Validation Report ---")
    ...
    # We'll check standard deviation of rho_train per trajectory is 0
    # Or just nunique.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--jsonl_path", type=str, default="datasets/price_domain.jsonl")
    parser.add_argument("--output_dir", type=str, default="datasets/processed_tables")
