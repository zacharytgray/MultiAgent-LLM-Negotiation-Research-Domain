...
class PriceState:
    ...
    timestep: int
    max_turns: int # Renamed from max_rounds for clarity (it is a turn limit)
    role: Literal["buyer", "seller"]
    last_offer_price: Optional[float]
    offer_history: List[Tuple[str, float]] # list of (proposer, price)
    effective_reservation_price: float # Fallback-adjusted threshold (point of indifference)
